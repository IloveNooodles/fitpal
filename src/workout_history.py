...
class WorkoutHistory(QWidget):
  ...
  def setUpWidgets(self):
    ...
    self.noHistoryFound.setText("You haven't completed any workout yet!\nPlease add a new workout to your history.")
    self.noHistoryFound.move(105, 208)
    self.noHistoryFound.setStyleSheet(f'color: rgba(255, 255, 255, 0.8); background-color:{bg_color}')
    ...
    self.addHistoryBtn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
  ...
  def setUpCards(self):
    ...
    for i in range(3):
      ...
      self.cards[i]['date'].setStyleSheet("background-color: #A19AFE; color: rgba(0, 0, 0, 0.5)")
      ...
      self.cards[i]['name'].setStyleSheet("background-color: #A19AFE; color: rgba(0, 0, 0, 0.5)")
      ...
      self.cards[i]['specification'].setStyleSheet("background-color: #A19AFE; color: rgba(0, 0, 0, 0.5)")
  
    # Set up left and right button
    self.leftButton = QPushButton(self)
    self.leftButton.setIcon(QIcon("images/left-btn.png"))
    ...
    self.rightButton.setIcon(QIcon("images/right-btn.png"))
    ...
    for i in range(3):
      ...
      if hIndex >= len(self.history):
        ...
        if self.history[hIndex]['olahraga_id'] == -1:
          self.cards[i]['cardImg'].setPixmap(QPixmap("images/weightlifting.png"))
